        ...
        <form method="POST" action="{{ route('timetables.store') }}">
            ...
            <div class="form-group mb-3">
                ...
                <select name="day_id" id="day_id" class="form-control" required>
                    ...
                    @foreach ($days as $id => $day)
                        <option value="{{ $id }}" {{ old('day_id') == $id ? 'selected' : '' }}>{{ $day }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <select name="hall_id" id="hall_id" class="form-control" required>
                    ...
                    @foreach ($halls as $id => $hall)
                        <option value="{{ $id }}" {{ old('hall_id') == $id ? 'selected' : '' }}>{{ $hall }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-row">
                <div class="form-group col-md-6 mb-3">
                    ...
                    <input type="time" name="time_from" id="time_from" class="form-control" value="{{ old('time_from') }}" required>
                </div>

                <div class="form-group col-md-6 mb-3">
                    <label for="time_to">Time To</label>
                    <input type="time" name="time_to" id="time_to" class="form-control" value="{{ old('time_to') }}" required>
                </div>
            </div>
